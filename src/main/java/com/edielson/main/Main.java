...
import com.edielson.component.Message;
import com.edielson.component.PanelCover;
import com.edielson.component.PanelLoading;
import com.edielson.component.PanelLoginAndRegister;
import com.edielson.component.PanelVerifyCode;
import com.edielson.model.ModelLogin;
...
public class Main extends javax.swing.JFrame {
    ...
    private void init() {
        ...
        bg.add(cover, "width " + COVER_SIZE + "%, pos 0al 0 n 100%");
        bg.add(loginAndRegister, "width " + LOGIN_SIZE + "%, pos 1al 0 n 100%"); //  1al as 100%
            ...
            public void actionPerformed(ActionEvent ae) {
                try {
                    ...
                    if (service.verifyCodeWithUser(user.getUserID(), verifyCode.getInputCode())) {
                        ...
                        showMessage(Message.MessageType.SUCCESS, "Conta criada");
                        verifyCode.setVisible(false);
                    } else {
                        showMessage(Message.MessageType.ERROR, "Código inválido");
                    }
    ...
    private void register() {
        ...
        try {
            if ((user.getUserName().isBlank() || user.getEmail().isBlank() || user.getPassword().isBlank())) {
                showMessage(Message.MessageType.ERROR, "Preencha todos os campos.");
            } else if (!emailValidator.isValid(user.getEmail())) {
                showMessage(Message.MessageType.ERROR, "Email inválido.");
            } else if (service.checkDuplicateUser(user.getUserName())) {
                showMessage(Message.MessageType.ERROR, "Nome já existe");
